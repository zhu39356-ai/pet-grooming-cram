...
function renderHome(){
  ...
  v.innerHTML=`
    ...
    ${!b.readyCommon?`<div class="banner"><b>共同科目題庫正在載入。</b><br><span class="small-text">目前 ${b.common}/400 題；連線時會自動同步四類共同科目。尚未完成前不會影響美容與急救作業。</span></div>`:''}
    ...
    <div class="section-title"><h2>題庫狀態</h2><button id="settingsBtn" class="ghost small">設定</button></div><div class="card"><div class="task"><div><b>13900 專業</b><span class="muted small-text">有效 ${b.professional} · 排除 ${b.excludedProfessional}</span></div><span class="pill ${b.readyProfessional?'good':'warn'}">647</span></div><div class="task"><div><b>共同科目</b><span class="muted small-text">每日 10 題＋丙級模考</span></div><span class="pill ${b.readyCommon?'good':'warn'}">${b.common}/400</span></div><div class="task"><div><b>寵物急救</b><span class="muted small-text">自建練習題 · 非官方考古題</span></div><span class="pill ${b.readyFirstAid?'good':'warn'}">${b.firstaid}/80</span></div></div>`;
  ...
  v.querySelector('#startCommonDaily')?.addEventListener('click',async()=>{
    if(!bankInfo().readyCommon){toast('正在載入共同科目，完成後會直接開始');try{await loadPdfJs();for(const src of SOURCES.common){if((bankInfo().byCommon[src.code]||0)<90)await syncOneCommon(src,()=>{});}await reloadQuestions();await ensureCycle('common');renderHome();await startDailyKind('common');}catch(e){console.error(e);toast('共同科目載入失敗，請到設定 → 同步／更新題庫重試');renderHome();}}else await startDailyKind('common');
  });
...
async function autoSync(){const btn=document.querySelector('#autoSyncBtn');btn.disabled=true;try{await loadBundledProfessional(syncMsg);await loadBundledFirstAid(syncMsg);await loadPdfJs();for(const src of SOURCES.common)await syncOneCommon(src,syncMsg);await reloadQuestions();await ensureCycle('professional');await ensureCycle('common');await ensureCycle('firstaid');syncMsg(`同步完成：美容 647 題、急救 80 題、共同 ${bankInfo().common} 題。永久排除清單已保留。`);renderSyncStatus();toast('題庫同步完成');renderHome();}catch(e){console.error(e);syncMsg(`同步沒有完成：${e.message}`);}finally{btn.disabled=false;}}
...
async function syncCommon(){const btn=document.querySelector('#syncCommonBtn');btn.disabled=true;try{await loadPdfJs();for(const src of SOURCES.common)await syncOneCommon(src,syncMsg);await reloadQuestions();await ensureCycle('common');renderSyncStatus();toast('共同科目更新完成');}catch(e){syncMsg(`共同科目同步失敗：${e.message}`);}finally{btn.disabled=false;}}

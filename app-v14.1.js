const SOURCES = {
  ...
  common: [
    {code:'90006',label:'職業安全衛生',expected:100,version:'V114101518'},
    {code:'90007',label:'工作倫理與職業道德',expected:100,version:'V114090117'},
    {code:'90008',label:'環境保護',expected:99,version:'V115041316'},
    {code:'90009',label:'節能減碳',expected:100,version:'V114112111'}
  ],
  history: [
...
const BUNDLED_COMMON_URL='./data/common-90006-90009.json';
...
async function loadBundledCommon(status=()=>{}){
  status('載入老師提供的 4 科共同題庫…');
  const r=await fetch(BUNDLED_COMMON_URL,{cache:'no-store'});
  if(!r.ok)throw new Error(`內建共同題庫 HTTP ${r.status}`);
  const data=await r.json();
  const all=Array.isArray(data?.questions)?data.questions:[];
  if(all.length!==400)throw new Error(`共同題庫來源只有 ${all.length}/400 題`);
  const active=all.filter(q=>q.active!==false);
  for(const src of SOURCES.common){
    const arr=active.filter(q=>q.subjectCode===src.code);
    if(arr.length!==src.expected)throw new Error(`${src.label} 有效題數 ${arr.length}/${src.expected}`);
  }
  const bad=active.find(q=>!q.id||!q.prompt||!Array.isArray(q.options)||q.options.length!==4||!q.options.every(Boolean)||![1,2,3,4].includes(Number(q.answer)));
  if(bad)throw new Error(`共同題庫格式異常：${bad.id||'未知題號'}`);
  const old=(await getAll(STORE_Q)).filter(q=>q.kind==='common');
  await new Promise((resolve,reject)=>{
    const t=db.transaction(STORE_Q,'readwrite'),store=t.objectStore(STORE_Q);
    for(const q of old)store.delete(q.id);
    for(const q of all)store.put(q);
    t.oncomplete=resolve;t.onerror=()=>reject(t.error);t.onabort=()=>reject(t.error||new Error('共同題庫更新中止'));
  });
  await reloadQuestions();
  await setMeta('bundledCommonVersion',APP_VERSION);
  await setMeta('commonTeacherBank',{at:new Date().toISOString(),sourceQuestionCount:400,activeQuestionCount:active.length,source:'teacher-provided-official-pdfs'});
  status(`共同科目已載入：${active.length} 題（官方刪題 1 題不出題）`);
  return active;
}
...
function dailyMode(kind){if(kind==='firstaid')return 'daily-firstaid';if(kind==='common')return 'daily-common';return 'daily-professional';}
...
function professionalOrderedPool(){
  const order=new Map(SOURCES.professional.sections.map(([code],i)=>[String(code),i]));
  return poolForKind('professional').slice().sort((a,b)=>
    (order.get(String(a.section))??999)-(order.get(String(b.section))??999) ||
    (Number(a.number)||0)-(Number(b.number)||0) || String(a.id).localeCompare(String(b.id))
  );
}
function professionalFirstPassInfo(){
  const pool=professionalOrderedPool(),unseen=pool.filter(q=>exposureCount(getProgress(q.id))===0);
  return {total:pool.length,seen:pool.length-unseen.length,remaining:unseen.length,complete:unseen.length===0,unseen};
}
async function ensureProfessionalReviewCycle(){
  if(!getMeta('professionalReviewCycleInitialized',false)){
    const ids=professionalOrderedPool().map(q=>q.id);
    await setMeta(cycleKey('professional'),{cycle:1,remaining:shuffle(ids),seen:[],startedAt:new Date().toISOString()});
    await setMeta('professionalReviewCycleInitialized',true);
  }
  return ensureCycle('professional',{renewIfComplete:true});
}
...
async function startDailyKind(kind){
  const target=kind==='firstaid'?Number(getMeta('dailyFirstAid',20)):kind==='common'?Number(getMeta('dailyCommon',10)):Number(getMeta('dailyProfessional',50)),done=dailyAnsweredToday(kind);if(done>=target){toast('今天這份作業已完成');return;}
  const need=Math.max(0,target-done);let qs=[];
  if(kind==='professional'){
    const first=professionalFirstPassInfo();
    if(!first.complete){
      if(getMeta('professionalReviewCycleInitialized',false))await setMeta('professionalReviewCycleInitialized',false);
      qs=first.unseen.slice(0,Math.min(need,first.unseen.length));
    }else{
      const d=await ensureProfessionalReviewCycle(),map=new Map(professionalOrderedPool().map(q=>[q.id,q]));
      qs=d.remaining.slice(0,Math.min(need,d.remaining.length)).map(id=>map.get(id)).filter(Boolean);
    }
  }else{
    const d=await ensureCycle(kind,{renewIfComplete:true}),map=new Map(poolForKind(kind).map(q=>[q.id,q]));
    qs=d.remaining.slice(0,Math.min(need,d.remaining.length)).map(id=>map.get(id)).filter(Boolean);
  }
  if(!qs.length){toast('目前沒有可出的題目');return;}startQuiz(qs,dailyMode(kind),kind==='firstaid'?'寵物急救每日作業':kind==='common'?'共同科目每日作業':'寵物美容丙級每日作業');
}
async function recordAnswer(q,chosen,opts={}){
  ...
  if(mode==='daily-professional'&&getMeta('professionalReviewCycleInitialized',false))await consumeCycle('professional',trackId);if(mode==='daily-firstaid')await consumeCycle('firstaid',trackId);if(mode==='daily-common')await consumeCycle('common',trackId);
  return {correct:!unknown&&chosen===q.answer,unknown};
}
function planFor(kind){
  const target=kind==='firstaid'?Number(getMeta('dailyFirstAid',20)):kind==='common'?Number(getMeta('dailyCommon',10)):Number(getMeta('dailyProfessional',50)),done=dailyAnsweredToday(kind);
  if(kind==='professional'){
    const first=professionalFirstPassInfo();
    if(!first.complete)return {target,done,remainingToday:Math.max(0,target-done),cycle:{cycle:1,remaining:first.remaining,seen:first.seen,total:first.total,complete:false,phase:'first-pass'}};
    return {target,done,remainingToday:Math.max(0,target-done),cycle:{...cycleInfo(kind),phase:'review'}};
  }
  return {target,done,remainingToday:Math.max(0,target-done),cycle:cycleInfo(kind)};
}

function renderHome(){
  const v=document.querySelector('#view-home'),b=bankInfo(),pro=planFor('professional'),common=planFor('common'),fa=planFor('firstaid'),proToday=todaysSummary('professional'),commonToday=todaysSummary('common'),faToday=todaysSummary('firstaid'),proWrong=pendingReview('professional').length,commonWrong=pendingReview('common').length,faWrong=pendingReview('firstaid').length,oc=overallCorrect();
  v.innerHTML=`
    <div class="hero"><div class="eyebrow" style="color:#ded9ff">TODAY</div><h2>今天要刷哪一套？</h2><p>美容專業、共同科目和寵物急救分開計算；答錯或按「不知道」的題目會留下來給你複習。</p><div class="metrics"><div class="metric"><b>${streakDays()}</b><small>連續天數</small></div><div class="metric"><b>${answeredUnique()}</b><small>已做過題目</small></div><div class="metric"><b>${oc.rate}%</b><small>作答正確率</small></div></div></div>
    ...
    ${!b.readyCommon?`<div class="banner"><b>共同科目題庫正在載入。</b><br><span class="small-text">目前 ${b.common}/399 題；本版會自動載入老師提供的四類共同科目。尚未完成前不會影響美容與急救作業。</span></div>`:''}
    <div class="section-title"><h2>三份每日作業</h2><span class="pill">互不混題</span></div>
    <div class="card"><div class="task"><div><b>寵物美容丙級</b><span class="muted small-text">今日 ${pro.done}/${pro.target} · ${pro.cycle.phase==='first-pass'?`第一輪順序刷題 · 已完成 ${pro.cycle.seen}/${pro.cycle.total}`:`隨機複習 · 第 ${pro.cycle.cycle} 輪 · 本輪剩 ${pro.cycle.remaining}/${pro.cycle.total}`}</span></div><span class="pill ${proWrong?'bad':'good'}">待複習 ${proWrong}</span></div><div class="progress"><i style="width:${pct(pro.cycle.seen,pro.cycle.total)}%"></i></div><p class="muted small-text">原始 647 題；已永久排除 ${b.excludedProfessional} 題。第一輪會依題庫順序只出「從未做過」的題目，保留既有紀錄並自動跳過已做題；全部完成後才進入隨機複習。</p><button id="startProDaily" class="primary wide" ${!b.readyProfessional||pro.done>=pro.target?'disabled':''}>${pro.done>=pro.target?'今天已完成':'開始／繼續美容作業'}</button></div>
    <div class="card"><div class="task"><div><b>共同科目</b><span class="muted small-text">今日 ${common.done}/${common.target} · 第 ${common.cycle.cycle} 輪 · 本輪剩 ${common.cycle.remaining}/${common.cycle.total}</span></div><span class="pill ${commonWrong?'bad':'good'}">待複習 ${commonWrong}</span></div><div class="progress"><i style="width:${pct(common.cycle.seen,common.cycle.total)}%"></i></div><p class="muted small-text">職安、工作倫理、環保、節能減碳共 4 類；每日 10 題，整輪刷完前不重複。</p><button id="startCommonDaily" class="secondary wide" ${common.done>=common.target?'disabled':''}>${common.done>=common.target?'今天已完成':b.readyCommon?'開始／繼續共同科目':'載入共同科目後開始'}</button></div>
    ...
    <div class="card"><h3>今天進度</h3><p class="muted small-text">美容：答對 ${proToday.correct} · 答錯 ${proToday.wrong} · 不知道 ${proToday.unknown}<br>共同：答對 ${commonToday.correct} · 答錯 ${commonToday.wrong} · 不知道 ${commonToday.unknown}<br>急救：答對 ${faToday.correct} · 答錯 ${faToday.wrong} · 不知道 ${faToday.unknown}</p></div>
    <div class="section-title"><h2>快速入口</h2></div><div class="grid2"><button id="goReview" class="secondary">錯題複習 (${proWrong+commonWrong+faWrong})</button><button id="goHistory" class="ghost">歷屆試題</button></div>
    <div class="section-title"><h2>題庫狀態</h2><button id="settingsBtn" class="ghost small">設定</button></div><div class="card"><div class="task"><div><b>13900 專業</b><span class="muted small-text">有效 ${b.professional} · 排除 ${b.excludedProfessional}</span></div><span class="pill ${b.readyProfessional?'good':'warn'}">647</span></div><div class="task"><div><b>共同科目</b><span class="muted small-text">每日 10 題＋丙級模考</span></div><span class="pill ${b.readyCommon?'good':'warn'}">${b.common}/399</span></div><div class="task"><div><b>寵物急救</b><span class="muted small-text">自建練習題 · 非官方考古題</span></div><span class="pill ${b.readyFirstAid?'good':'warn'}">${b.firstaid}/80</span></div></div>`;
  v.querySelector('#startProDaily')?.addEventListener('click',()=>startDailyKind('professional'));
  v.querySelector('#startCommonDaily')?.addEventListener('click',async()=>{
    if(!bankInfo().readyCommon){toast('正在載入共同科目，完成後會直接開始');try{await loadBundledCommon(()=>{});await reloadQuestions();await ensureCycle('common');renderHome();await startDailyKind('common');}catch(e){console.error(e);toast('共同科目載入失敗，請到設定 → 同步／更新題庫重試');renderHome();}}else await startDailyKind('common');
  });
  v.querySelector('#startFaDaily')?.addEventListener('click',()=>startDailyKind('firstaid'));v.querySelector('#goReview').onclick=()=>navigate('review');v.querySelector('#goHistory').onclick=()=>renderHistory(true);v.querySelector('#settingsBtn').onclick=()=>renderSettings(true);
}
...
function renderStats(){
  const v=document.querySelector('#view-stats'),b=bankInfo(),pro=overallCorrect('professional'),common=overallCorrect('common'),fa=overallCorrect('firstaid'),proPending=pendingReview('professional').length,commonPending=pendingReview('common').length,faPending=pendingReview('firstaid').length;
  ...
  v.innerHTML=`<div class="section-title"><h2>學習紀錄</h2><span class="pill">錯題優先</span></div><div class="card"><h3>美容 ${pro.rate}%</h3><span class="muted small-text">已做 ${answeredUnique('professional')}/${b.professional} · 待複習 ${proPending}</span></div><div class="card"><h3>共同科目 ${common.rate}%</h3><span class="muted small-text">已做 ${answeredUnique('common')}/${b.common} · 待複習 ${commonPending}</span></div><div class="card"><h3>急救 ${fa.rate}%</h3><span class="muted small-text">已做 ${answeredUnique('firstaid')}/${b.firstaid} · 待複習 ${faPending}</span></div><div class="card"><h3>美容各項目</h3>${rows('professional')}</div><div class="card"><h3>共同科目各類</h3>${rows('common')}</div><div class="card"><h3>急救各主題</h3>${rows('firstaid')}</div><div class="card"><h3>永久排除</h3><p class="muted small-text">美容 ${b.excludedProfessional} 題 · 急救 ${b.excludedFirstAid} 題。排除後不會再被題庫更新補回每日作業。</p></div>`;
}
...
function renderSettings(navigateNow=false){
  ...
  const v=document.querySelector('#view-settings');v.innerHTML=`<div class="section-title"><h2>設定</h2><span class="pill">${APP_VERSION}</span></div><div class="card"><div class="setting-row"><div><b>每日美容題數</b><div class="muted small-text">第一輪依題庫順序補齊未做題；全部完成後才隨機複習</div></div><input id="dailyPro" type="number" min="5" max="100" value="${getMeta('dailyProfessional',50)}"></div><div class="setting-row"><div><b>每日共同科目題數</b><div class="muted small-text">職安、倫理、環保、節能四類混合，整輪不重複</div></div><input id="dailyCommon" type="number" min="5" max="40" value="${getMeta('dailyCommon',10)}"></div><div class="setting-row"><div><b>每日急救題數</b><div class="muted small-text">與美容作業完全分開</div></div><input id="dailyFa" type="number" min="5" max="80" value="${getMeta('dailyFirstAid',20)}"></div></div><div class="card"><h3>刷題規則</h3><p class="muted small-text">美容原始 647 題會先扣除永久排除題。第一輪完全依官方題庫順序前進，系統會讀取既有作答紀錄，已做過的題目直接跳過，只補從未做過的題目；第一輪全部完成後才切換成隨機複習。文字選項仍會隨機換位置，避免只背 A/B/C/D。</p></div><div class="card"><h3>永久排除題目</h3><p class="muted small-text">固定不考 ${DEFAULT_EXCLUDED_IDS.length} 題（13900-06：11、12、14、15、18–23、25–42）；另外自行排除 ${customEx.length} 題。題庫同步、App 更新與清除學習紀錄都不會把固定不考題加回來。</p><button id="restoreExcluded" class="ghost wide" ${!customEx.length?'disabled':''}>恢復自行排除題 (${customEx.length})</button></div><div class="card"><h3>自動保存與備份</h3><p><b>✓ 每一題作答後自動保存</b></p><div class="sync-line"><span>最近自動快照</span><b>${esc(lastText)}</b></div><div class="sync-line"><span>瀏覽器永久儲存</span><b>${persist?'已啟用':'未確認'}</b></div><div class="grid2" style="margin-top:12px"><button id="exportBtn" class="secondary">匯出備份</button><button id="importBackupBtn" class="ghost">匯入備份</button></div><input id="backupFile" type="file" accept="application/json" hidden></div><div class="card"><h3>題庫</h3><button id="syncSettings" class="primary wide">同步／更新題庫</button></div><div class="card"><h3>危險區</h3><button id="resetBtn" class="danger wide">清除學習紀錄（保留排除題）</button></div><button id="settingsHome" class="ghost wide">回今日首頁</button>`;
  v.querySelector('#dailyPro').onchange=async e=>{await setMeta('dailyProfessional',clamp(Number(e.target.value)||50,5,100));queueAutoSnapshot('設定更新');};v.querySelector('#dailyCommon').onchange=async e=>{await setMeta('dailyCommon',clamp(Number(e.target.value)||10,5,40));queueAutoSnapshot('設定更新');};v.querySelector('#dailyFa').onchange=async e=>{await setMeta('dailyFirstAid',clamp(Number(e.target.value)||20,5,80));queueAutoSnapshot('設定更新');};v.querySelector('#syncSettings').onclick=openSyncDialog;v.querySelector('#settingsHome').onclick=()=>navigate('home');v.querySelector('#exportBtn').onclick=exportBackup;v.querySelector('#importBackupBtn').onclick=()=>v.querySelector('#backupFile').click();v.querySelector('#backupFile').onchange=importBackup;
  ...
  v.querySelector('#resetBtn').onclick=()=>confirmAction('清除學習紀錄？','作答、錯題與成績會歸零；永久排除清單會保留。',async()=>{await clearStore(STORE_P);state.progress.clear();await setMeta('mockResults',[]);await setMeta(cycleKey('professional'),null);await setMeta('professionalReviewCycleInitialized',false);await setMeta(cycleKey('common'),null);await setMeta(cycleKey('firstaid'),null);await ensureCycle('professional');await ensureCycle('common');await ensureCycle('firstaid');await writeAutoSnapshot('清除後快照');toast('學習紀錄已清除');renderSettings();});
}
...
async function autoSync(){const btn=document.querySelector('#autoSyncBtn');btn.disabled=true;try{await loadBundledProfessional(syncMsg);await loadBundledFirstAid(syncMsg);await loadBundledCommon(syncMsg);await reloadQuestions();await ensureCycle('professional');await ensureCycle('common');await ensureCycle('firstaid');syncMsg(`同步完成：美容 647 題、急救 80 題、共同 ${bankInfo().common} 題（官方刪題 1 題不出題）。永久排除清單已保留。`);renderSyncStatus();toast('題庫同步完成');renderHome();}catch(e){console.error(e);syncMsg(`同步沒有完成：${e.message}`);}finally{btn.disabled=false;}}
...
async function syncCommon(){const btn=document.querySelector('#syncCommonBtn');btn.disabled=true;try{await loadBundledCommon(syncMsg);await reloadQuestions();await ensureCycle('common');renderSyncStatus();toast('共同科目更新完成');}catch(e){syncMsg(`共同科目同步失敗：${e.message}`);}finally{btn.disabled=false;}}
...
async function refreshBundledBanksInBackground(){
  try{
    ...
    const commonCount=state.questions.filter(q=>q.kind==='common').length;
    if(commonCount<399||getMeta('bundledCommonVersion')!==APP_VERSION){
      await loadBundledCommon(()=>{});
    }
    await reloadQuestions();await ensureCycle('professional');await ensureCycle('common');await ensureCycle('firstaid');renderHome();
  }catch(e){

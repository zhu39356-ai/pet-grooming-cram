...
async function recordAnswer(q,chosen,opts={}){
  ...
  if(mode==='daily-professional')await consumeCycle('professional',trackId);if(mode==='daily-firstaid')await consumeCycle('firstaid',trackId);if(mode==='daily-common')await consumeCycle('common',trackId);
  return {correct:!unknown&&chosen===q.answer,unknown};
}
function planFor(kind){const target=kind==='firstaid'?Number(getMeta('dailyFirstAid',20)):kind==='common'?Number(getMeta('dailyCommon',10)):Number(getMeta('dailyProfessional',50)),done=dailyAnsweredToday(kind),cycle=cycleInfo(kind);return {target,done,remainingToday:Math.max(0,target-done),cycle};}
...
function renderSettings(navigateNow=false){
  ...
  v.querySelector('#resetBtn').onclick=()=>confirmAction('清除學習紀錄？','作答、錯題與成績會歸零；永久排除清單會保留。',async()=>{await clearStore(STORE_P);state.progress.clear();await setMeta('mockResults',[]);await setMeta(cycleKey('professional'),null);await setMeta(cycleKey('common'),null);await setMeta(cycleKey('firstaid'),null);await ensureCycle('professional');await ensureCycle('common');await ensureCycle('firstaid');await writeAutoSnapshot('清除後快照');toast('學習紀錄已清除');renderSettings();});
}
